...
observes that singular object's type conversion. With **hop itc** it's
**handled** __on__ an object's `prototype` and observes type conversions for all
objects of that type. An **ion** can implement either style or alternate between
both.
...
**hip hop itc** is a simple & reliable capability for independently observing &
notifying objects. It can be implemented manually, as shown, or leveraged via
